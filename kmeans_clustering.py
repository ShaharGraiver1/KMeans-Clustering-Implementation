...
def kmean_flow():
    ...
    k = sys.argv[1]
    itr = "200" if input_len < 3 else sys.argv[2]
    input_data = sys.argv[2] if input_len < 3 else sys.argv[3]

    with open(input_data, 'r') as file:
        error_cnt = 0
        vector_list = file.readlines()
        line_count = len(vector_list)
    if k.isnumeric() is False or int(k) >= (line_count - 1) or int(k) <= 1:  # check if: 1. k is numeric 2. 1<k<n
        print("Invalid number of clusters!")
        error_cnt += 1
    if itr.isnumeric() is False or int(itr) >= 1000 or int(itr) <= 1:  # check if: 1.itr is numeric 2. 1<itr<1000
